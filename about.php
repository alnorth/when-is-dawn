<!DOCTYPE html>
<html>
	<head>
		<title>When is Dawn?</title>
		<link rel="stylesheet" href="style.css" type="text/css" />
		<?php include('hidden/ga.php'); ?>
	</head>
	<body>
		<div class="wrapper">
			<div id="about">
				<p>This website tells you when the 'crack of dawn' is wherever you are. If it's not right (and for some reason you're really concerned about that) then contact me on twitter to let me know.</p>
			</div>
			<div class="push"></div>
		</div>
		<div id="footer">
			by <a href="http://www.twitter.com/alasdairnorth">@alasdairnorth</a> | <a href="about.php">about</a> | <a href="http://software.alnorth.com/">other projects</a>
		</div>
	</body>
</html>