...
function hostip_get_info($ip) {
	...
	if ($content != FALSE) {
		...
		$ipLocation = $xml->children('gml', TRUE)->featureMember->children('', TRUE)->Hostip->ipLocation;
		if($ipLocation != "") {
			$coordinates = $ipLocation->children('gml', TRUE)->pointProperty->Point->coordinates;
